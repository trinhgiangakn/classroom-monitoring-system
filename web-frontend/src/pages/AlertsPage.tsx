...
export function AlertsPage() {
  ...
  const isAdmin = decodeJwtPayload(localStorage.getItem('accessToken') || '')?.role === 'admin'
  ...
  return (
    <section>
      ...
      {selected ? <AlertDialog alert={selected} dismissed={filter === 'dismissed'} isAdmin={isAdmin} onAcknowledge={() => void updateAlert('acknowledge')} onClose={() => setSelected(null)} onDelete={() => void removeSelected()} onResolve={() => void updateAlert('resolve')} onRestore={() => void restoreSelected()} /> : null}
    </section>
  )
}
